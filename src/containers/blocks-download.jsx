// 暂时先放弃此方案，涉及扩展加载，项目缓存加载，文件保存，文件打开
// 未来优化
...
class BlocksDownload extends React.Component {
    constructor (props) {
        super(props);
        this.ScratchBlocks = VMScratchBlocks(props.vm, false);//初始化 ScratchBlocks
    ...
    componentDidMount () {
        ...
        toolboxWorkspace.registerButtonCallback('MAKE_A_VARIABLE', varListButtonCallback(''));//创建变量/列表
        toolboxWorkspace.registerButtonCallback('MAKE_A_LIST', varListButtonCallback('list'));
        toolboxWorkspace.registerButtonCallback('MAKE_A_PROCEDURE', procButtonCallback);//创建自定义积木
        toolboxWorkspace.registerButtonCallback('EXTENSION_CALLBACK', block => {//扩展积木按钮事件
            this.props.vm.handleExtensionButtonPress(block.callbackData_);
        });
        toolboxWorkspace.registerButtonCallback('OPEN_EXTENSION_DOCS', block => {//打开扩展文档
        ...
        toolboxWorkspace.registerButtonCallback('OPEN_RETURN_DOCS', () => {//打开函数返回文档
        ...
        // 当用户拖动或缩放 workspace 时触发 onWorkspaceMetricsChange，用于更新 UI 或布局。
        addFunctionListener(this.workspace, 'translate', this.onWorkspaceMetricsChange);
        addFunctionListener(this.workspace, 'zoom', this.onWorkspaceMetricsChange);

        //设置 VM 编译器选项
        this.props.vm.setCompilerOptions({
            warpTimer: true//true → 加速 VM 的执行
        });

        this.attachVM();//绑定 VM 到组件 订阅 VM 事件，保证 React 和 ScratchBlocks 的状态联动。
        ...
        //处理扩展和持久存储
        for (const category of this.props.vm.runtime._blockInfo) {//遍历VM 中已注册的 block category
           console.log(category)
            this.handleExtensionAdded(category);
        }

        gentlyRequestPersistentStorage();//请求持久存储
    }
    ...
    shouldComponentUpdate (nextProps, nextState) {
        return (
            this.state.prompt !== nextState.prompt ||//控制弹出输入框（Prompt）是否变化。
            this.props.isVisible !== nextProps.isVisible ||//当前积木编辑器是否可见
            this._renderedToolboxXML !== nextProps.toolboxXML ||//工具箱 XML 是否变化
            this.props.extensionLibraryVisible !== nextProps.extensionLibraryVisible ||//扩展库
            this.props.customProceduresVisible !== nextProps.customProceduresVisible ||//自定义函数面板显示状态
            this.props.locale !== nextProps.locale ||//语言
            this.props.anyModalVisible !== nextProps.anyModalVisible ||//任何模态框打开状态
            this.props.stageSize !== nextProps.stageSize ||//舞台尺寸变化
    ...
    componentDidUpdate (prevProps) {
        ...
        // 仅当 blocks 可见且工具箱 XML 变化时重新渲染工具箱
        ...
        if (this.props.isVisible) { // Scripts tab
            ...
            if (prevProps.locale !== this.props.locale || this.props.locale !== this.props.vm.getLocale()) {
                ...
                this.setLocale();//切换语言时刷新 workspace
            } else {
    ...
    componentWillUnmount () {
        this.detachVM();//解绑 VM
        this.unmounted = true;
        this.workspace.dispose();//销毁 Blockly workspace
        clearTimeout(this.toolboxUpdateTimeout);//清除可能存在的延迟调用

        // Clear the flyout blocks so that they can be recreated on mount.
        this.props.vm.clearFlyoutBlocks();//清理 VM 中的 flyout blocks
...
BlocksDownload.defaultProps = {
    isVisible: true,
    options: BlocksDownload.defaultOptions,
    theme: Theme.light
};
...
export default injectIntl(errorBoundaryHOC('Blocks')(
    connect(
        mapStateToProps,
        mapDispatchToProps
    )(LoadScratchBlocksHOC(BlocksDownload))
));